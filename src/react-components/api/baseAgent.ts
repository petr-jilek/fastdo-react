...
export interface Error_ErrorModel {
...
const handleError = (error: any, showToast: boolean, push: boolean): Promise<any> => {
    if (axios.isAxiosError(error)) {
        ...
        if (isErrorModel(data)) {
            var errorModel = data as Error_ErrorModel
...
const handleErrorModel = (status: number, error: Error_ErrorModel, showToast: boolean, push: boolean) => {
    switch (status) {
        case 401:
...
export const requests = {
    ...
    post: <T>(url: string, body: any = {}, showToast = true, push = true): Promise<T> => {
